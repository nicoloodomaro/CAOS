...
{
    (void) pvArg;
    printf("[TASK] HRT_C start tick=%u\r\n", (unsigned int) xTaskGetTickCount());
    vBusyWaitMs(1U);
    printf("[TASK] HRT_C end   tick=%u\r\n", (unsigned int) xTaskGetTickCount());
}
...
static const TimelineTaskConfig_t xTasks[] = {
    /*
     * PREEMPT profile:
     * - sf0: HRT_A at t=0..1ms, then SRT starts.
     * - sf0: HRT_B at t=3..4ms while SRT_LONG is still running.
     * - sf1: HRT_C at t=1..3ms, causing another interruption.
     */
